...
#define THINGSPEAK_API_WRITEKEY "YOUR_THINGSPEAK_API_WRITEKEY" // Not used in this project
...
// Thingspeak.com API
TCPClient client;
const char * WRITEKEY = THINGSPEAK_API_WRITEKEY; // Not used in this project
...
void display_string_5x7(byte y,byte x,const char *text)
{
  ...
  while((text[i]>0x00))
  {
    ...
    {           
      ...
      display_graphic_5x7(y,x+1,fontbuf);
      i+=1;
      x+=6;
    }
    else
...
void loop()
{
    refreshClock();
    if (colorChanged) {
        colorWipe(CheerLightsColor, 20);
        colorChanged = false;
    }
    if (millis() - lastread > RAINBOWTIME) {
        if ((millis() - lastread) > (RAINBOWTIME + RAINBOWLENGTH)) {
            lastread = millis();
            colorChanged = true;
        }
        if (!colorChanged) rainbow(20);
    } else {
        delay(200);
    }
}
